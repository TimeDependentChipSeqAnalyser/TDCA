...
std::string drcR(int i_loopNumber, std::string s_peakExtention, bool b_L4flag)  
{
	...
	**/
	...
	if (b_L4flag) {
		// L4 function : asymetry factor (f) = 1
		s_rCommand += "try(peak" + s_EX + ".L.5 <- drm(depth" + s_EX + " ~ time" + s_EX + ", data = peak" + s_EX + ", fct = L.5(fixed=c(NA,NA,NA,NA,1))))\n";
		s_rCommand += "try(peak" + s_EX + ".FL.5 <- drm(depth" + s_EX + " ~ time" + s_EX + ", data = peak" + s_EX + ", fct = L.5(fixed=c(NA,0,NA,NA,1))))\n";
		s_rCommand += "try(peak" + s_EX + ".FLMIN.5 <- drm(depth" + s_EX + " ~ time" + s_EX + ", data = peak" + s_EX + ", fct = L.5(fixed=c(NA,min(depth" + s_EX + "),NA,NA,1))))\n";
		s_rCommand += "try(Rpeak" + s_EX + ".L.5 <- drm(depth" + s_EX + " ~ time" + s_EX + ", data = peak" + s_EX + ", fct = L.5(fixed=c(NA,NA,max(depth" + s_EX + "),NA,1))))\n"; 
		s_rCommand += "try(Rpeak" + s_EX + ".FL.5 <- drm(depth" + s_EX + " ~ time" + s_EX + ", data = peak" + s_EX + ", fct = L.5(fixed=c(NA,0,max(depth" + s_EX + "),NA,1))))\n"; 
		s_rCommand += "try(Rpeak" + s_EX + ".FLMIN.5 <- drm(depth" + s_EX + " ~ time" + s_EX + ", data = peak" + s_EX + ", fct = L.5(fixed=c(NA,min(depth" + s_EX + "),max(depth" + s_EX + "),NA,1))))\n"; 
	} else {
		// L5 function
	...
	s_rCommand += "if ( (exists(\"peak" + s_EX + ".L.5\")) & (exists(\"peak" + s_EX + ".FL.5\")) & (exists(\"peak" + s_EX + ".FLMIN.5\")) & (exists(\"Rpeak" + s_EX +
		".L.5\")) & (exists(\"Rpeak" + s_EX + ".FL.5\")) & (exists(\"Rpeak" + s_EX + ".FLMIN.5\")) ) {\n"; // if all 6 models exist

	if (b_L4flag) { // Fix asymetry factor to 1
		// FL.5
		s_rCommand += "  if ( (peak" + s_EX + ".L.5$parmMat[2] < 0) & (peak" + s_EX + ".L.5$parmMat[1] < 0) & (peak" + s_EX + ".FL.5$parmMat[3] < (" + s_Mult + "*max(time" + s_EX + "))) ) {\n";
		s_rCommand += "    tryCatch(suppressWarnings(summary(peak" + s_EX + ".FL.5)), print(paste(\"c:(Intercept) FORCED\")), print(paste(\"f:(Intercept) 1\")), error = function(e)" +
		...
		s_rCommand += "    tryCatch(suppressWarnings(summary(peak" + s_EX + ".L.5)), print(paste(\"f:(Intercept) 1\")),error = function(e) {print(paste(\"e:(Intercept) ERROR\")); " +
		...
		// FLMIN.5
		s_rCommand += "  } else if ( (peak" + s_EX + ".L.5$parmMat[2] < 0) & (peak" + s_EX + ".L.5$parmMat[1] < 0) ) {\n";
		s_rCommand += "    tryCatch(suppressWarnings(summary(peak" + s_EX + ".FLMIN.5)), print(paste(\"f:(Intercept) 1\")), cat(\"c:(Intercept)\", min(depth" + s_EX + "), \"\\n\"), " +
			"error = function(e) {print(paste(\"e:(Intercept) ERROR\")); print(paste(\"d:(Intercept) ERROR\")); print(paste(\"c:(Intercept) ERROR\")); print(paste(\"b:(Intercept) ERROR\")); " +
			"print(paste(\"f:(Intercept) ERROR\")); print(paste(\" NaN (-1 degrees of freedom)\")); NaN})\n";
		// rFL.5
		s_rCommand += "  } else if ( (Rpeak" + s_EX + ".L.5$parmMat[2] < 0) & (Rpeak" + s_EX + ".FL.5$parmMat[2] < (" + s_Mult + "*max(time" + s_EX + "))) ) {\n";
		s_rCommand += "    tryCatch(suppressWarnings(summary(Rpeak" + s_EX + ".FL.5)), print(paste(\"f:(Intercept) 1\")), print(paste(\"c:(Intercept) FORCED\")), " +
			"cat(\"d:(Intercept)\", max(depth" + s_EX + "), \"\\n\"), error = function(e) {print(paste(\"e:(Intercept) ERROR\")); print(paste(\"d:(Intercept) ERROR\")); " +
		...
		s_rCommand += "    tryCatch(suppressWarnings(summary(Rpeak" + s_EX + ".L.5)), print(paste(\"f:(Intercept) 1\")), cat(\"d:(Intercept)\", max(depth" + s_EX + "), \"\\n\"), error = function(e) " +
		...
		// rFLMIN.5
	 	s_rCommand += "  } else {\n";
		s_rCommand += "    tryCatch(suppressWarnings(summary(Rpeak" + s_EX + ".FLMIN.5)), print(paste(\"f:(Intercept) 1\")), cat(\"d:(Intercept)\", max(depth" + s_EX + "), \"\\n\"), " +
			"cat(\"c:(Intercept)\", min(depth" + s_EX + "), \"\\n\"), error = function(e) {print(paste(\"e:(Intercept) ERROR\")); print(paste(\"d:(Intercept) ERROR\")); " +
			"print(paste(\"c:(Intercept) ERROR\")); print(paste(\"b:(Intercept) ERROR\")); print(paste(\"f:(Intercept) ERROR\")); print(paste(\" NaN (-1 degrees of freedom)\")); NaN})\n";
	 	s_rCommand += "  }\n";
