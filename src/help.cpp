...
int help()
{
	...
	std::cout << "		If replicates are available, they ust contain the same time points. Run replicates seperately if" << std::endl;
	...
	std::cout << "		columns specifying chromosome, start position and end potition. Ensure the bed file is sorted, such as:" << std::endl;
	...
	std::cout << "		User specified gene file containing RefSeq gene names. Each gene must be on a seperate line\n" << std::endl;
	...
	std::cout << "		This parameter is imortant in the calsification of loci type (hills, rises, valleys, and falls). Consequently," << std::endl;
	...
	std::cout << "		valleys, or falls. If not set, loci are models as a rise or fall only (sinlge 5 parameter sigmoid).\n" << std::endl;
