...
void inflectionWriterVersitile(int i_maxThreads, int i_peakNumber, std::vector<std::string> &s_drcFI_vec, std::vector<std::string> &s_drcFH_vec, std::vector<std::string> &s_drcFU_vec, std::vector<std::string> &s_drcFL_vec, std::vector<std::string> &s_drcFE_vec, std::vector<std::string> &s_drcRI_vec, std::vector<std::string> &s_drcRH_vec, std::vector<std::string> &s_drcRU_vec, std::vector<std::string> &s_drcRL_vec, std::vector<std::string> &s_drcRE_vec, std::vector<std::string> &s_type_vec)
{
	...
	try {
		if ( i_maxThreads == 1) {
			std::string s_drcScript = "Rscript drcVersitile.R > drcVersitile.txt";
			std::system(s_drcScript.c_str());
		} else {
			#pragma omp parallel for
			for (int i = 1; i < i_maxThreads+1; i++) {
				std::string s_drcScript = "Rscript drcVersitile." + to_string(i) + ".R > drcVersitile." + to_string(i) + ".txt";
				std::system(s_drcScript.c_str());
			}
		}
	} catch(...){
	...
	if ( i_maxThreads > 1) {
		...
		for (int i = 1; i < i_maxThreads+1; i++) { s_cat += "drcVersitile." + to_string(i) + ".txt "; }
		s_cat += "> drcVersitile.txt";
		std::system(s_cat.c_str());
		s_cat = "cat ";
		for (int i = 1; i < i_maxThreads+1; i++) { s_cat += "drcVersitile." + to_string(i) + ".R "; }
		s_cat += "> drcVersitile.R";
		std::system(s_cat.c_str());
		for (int i = 1; i < i_maxThreads+1; i++) { 
			std::string s_rm = "rm drcVersitile." + to_string(i) + ".txt; rm drcVersitile." + to_string(i) + ".R"; 
	...
	for (int i = 0; i < 5; i++) { 
		...
		std::string s_parameterScript = "grep -o '" + s_parameter + ":(Intercept)[[:space:]]*[^[:space:]]*'" +
			" drcVersitile.txt | grep -o '[[:space:]]*[^[:space:]]*' | grep -o '[^[:space:]]*' > parameter.txt";
