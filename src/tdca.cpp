...
int main(int argc, char* argv[])
{
	...
	if (b_genelistFlagCall == true && b_genomeFlagCall == false) {
		...
		std::exit(0); }
	if (b_genelistFlagCall == false && b_anime == false) {
	...
	if (b_L4flag) {
		std::cout << "-L4 flag called. TDCA will fix asymetry factor to 1 (L4)." << std::endl; }
	...
	dataArray[0][2*i_bamFiles + 3]  = "Loci Type"; 
	dataArray[0][2*i_bamFiles + 4]  = "forward TTI"; 
	dataArray[0][2*i_bamFiles + 5]  = "forward Hills coefficient";
	dataArray[0][2*i_bamFiles + 6]  = "forward upper asymptote"; 
	dataArray[0][2*i_bamFiles + 7]  = "forward lower asymptote";	
	dataArray[0][2*i_bamFiles + 8]  = "reverse TTI"; 
	dataArray[0][2*i_bamFiles + 9]  = "reverse Hills coefficient"; 
	dataArray[0][2*i_bamFiles + 10] = "reverse upper asymptote"; 
	dataArray[0][2*i_bamFiles + 11] = "reverse lower asymptote"; 
	...
	// write normalized gene reads to vector for animation
...
/**
	...
	if ( (b_satFlagCall == false) && (b_model == true) ) { // skipped if nomodel (no eliminated loci)
		...
		for (int k = 0; k < 5; k++) {
			...
			inflectionWriterVersitile(i_maxThreads, i_testLoci, s_drcFI_vec, s_drcFH_vec, s_drcFU_vec, s_drcFL_vec, s_drcFE_vec, s_drcRI_vec, s_drcRH_vec, s_drcRU_vec, s_drcRL_vec, s_drcRE_vec, s_type_vec, s_name);
			// ELIMINATE BAD LOCI
...
**/
	...
	inflectionWriterVersitile(i_maxThreads, i_peakNumber, s_drcFI_vec, s_drcFH_vec, s_drcFU_vec, s_drcFL_vec, s_drcFE_vec, s_drcRI_vec, s_drcRH_vec, s_drcRU_vec, s_drcRL_vec, s_drcRE_vec, s_type_vec, s_name);
