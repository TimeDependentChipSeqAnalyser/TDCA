...
**/
	...
	for (int i = 0; i < 25; i++) { // downstream
		if (s_strand == "-") {
			...
			i_downstart = atoi(s_start.c_str()) - (i_flank/25*i);
			i_downend = atoi(s_start.c_str()) - (i_flank/25*(i+1));
		...
		if (s_strand == "+") {
			...
			i_downstart = atoi(s_end.c_str()) + (i_flank/25*i);
			i_downend = atoi(s_end.c_str()) + (i_flank/25*(i+1));
