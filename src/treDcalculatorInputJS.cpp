...
void treDcalculatorInputJS(double transferArrJS[][1200], std::string s_bamPath, std::string s_geneinfo, std::string s_inputBamPath, double d_minDepth, double d_minInputDepth, double depthArr[], double inputDepthArr[], int j) {
	...
	for (int i = 0; i < 200; i++) { // upstream
		if (s_strand == "-") {
			...
			// CALCULATE DEPTH: subtract input
			d_depth = (samDregion(s_bamPath, s_upCoordinates) / depthArr[j] * d_minDepth) - (samDregion(s_inputBamPath, s_upCoordinates) / inputDepthArr[j] * d_minInputDepth);
		}
		if (s_strand == "+") {
	...
	for (int i = 0; i < 200; i++) { // downstream
		if (s_strand == "-") {
			...
			// CALCULATE DEPTH: subtract input
			d_depth = (samDregion(s_bamPath, s_downCoordinates) / depthArr[j] * d_minDepth) - (samDregion(s_inputBamPath, s_downCoordinates) / inputDepthArr[j] * d_minInputDepth);
		}
		if (s_strand == "+") {
	...
	for (int i = 0; i < 800; i++) { // gene body
		if (s_strand == "-") {
			...
			// CALCULATE DEPTH: subtract input
			d_depth = (samDregion(s_bamPath, s_geneCoordinates) / depthArr[j] * d_minDepth) - (samDregion(s_inputBamPath, s_geneCoordinates) / inputDepthArr[j] * d_minInputDepth);
		}	
		if (s_strand == "+") {
